...
func (m *TestRequest) MarshalTo(dAtA []byte) (int, error) {
	...
	if len(m.Msg) > 0 {
		dAtA[i] = 0xa
		i++
		i = encodeVarintTestrequest(dAtA, i, uint64(len(m.Msg)))
		i += copy(dAtA[i:], m.Msg)
	}
	return i, nil
}

func encodeVarintTestrequest(dAtA []byte, offset int, v uint64) int {
	for v >= 1<<7 {
...
func (m *TestRequest) Size() (n int) {
	...
	l = len(m.Msg)
	if l > 0 {
		n += 1 + l + sovTestrequest(uint64(l))
	}
	return n
}

func sovTestrequest(x uint64) (n int) {
	for {
		n++
		x >>= 7
		if x == 0 {
...
func sozTestrequest(x uint64) (n int) {
	return sovTestrequest(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (m *TestRequest) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowTestrequest
			}
			if iNdEx >= l {
		...
		default:
			iNdEx = preIndex
			skippy, err := skipTestrequest(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
...
func skipTestrequest(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowTestrequest
			}
			if iNdEx >= l {
		...
		case 3:
			for {
				...
				next, err := skipTestrequest(dAtA[start:])
				if err != nil {
					return 0, err
				}
				iNdEx = start + next
...
var fileDescriptor_testrequest_2a8563a6fbbdc37a = []byte{
	...
	0x73, 0x8b, 0xd3, 0x25, 0x18, 0x15, 0x18, 0x35, 0x38, 0x83, 0x40, 0x4c, 0x27, 0x81, 0x13, 0x8f,
	0xe4, 0x18, 0x2f, 0x3c, 0x92, 0x63, 0x7c, 0xf0, 0x48, 0x8e, 0x71, 0xc2, 0x63, 0x39, 0x86, 0x24,
	0x36, 0xb0, 0x7e, 0x63, 0x40, 0x00, 0x00, 0x00, 0xff, 0xff, 0xc8, 0x32, 0x07, 0x5c, 0x54, 0x00,
	0x00, 0x00,
}
