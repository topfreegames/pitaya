...
由于游戏在场景管理、客户端动画等方面有一定的复杂性，并不适合作为`pitaya`的入门应用。对于大多数开发者
而言，普通聊天室是一个更加适合入门`pitaya`的应用。

`pitaya`是一个轻量级的服务器框架，它最适合的应用领域是网页游戏、社交游戏、移动游戏的服务端。当然还不
仅仅是游戏，用`pitaya`开发高实时web应用也非常合适。

## 前言

- 本教程适用于对`pitaya`零基础的用户，如果你已经有过一定的`pitaya`开发基础，请跳过这个教程，你可以阅读
开发指南，那里会对一些话题作较为详细的探讨。

- 由于`pitaya`是基于Go开发的，因此希望你在阅读本教程前对Go语言有一些了解。

- 本教程的示例源码放在github上[完整代码](https://github.com/topfreegames/pitaya/tree/master/examples/demo/chat)

- 本教程将以一个实时聊天应用为例子，通过对这个应用进行不同的修改来展示`pitaya`框架的一些功能特性，让用
户能大致了解`pitaya`，熟悉并能够使用`pitaya`进行应用程序的开发。
...
`pitaya`有一些自己的术语，这里先对术语做一些简单的解释，给读者一个直观的概念，不至于看到相应术语时产生
迷惑。

### 组件(Component)

`pitaya`应用是由一些松散耦合的`Component`组成的，每个`Component`完成一些功能。整个应用可以看作是一
...
`Room`中的`Handler`, `Room`中所有符合`Handler`签名的方法都会在`pitaya`应用启动时自动注册.
...
`pitaya`中有四种消息类型的消息，分别是请求(Request), 响应(Response), 通知(Notify)和推送(Push)，客
...
这部分, 我们构建了一个简单的聊天应用, 并对代码做了简单的介绍, 通过这个教程, 相信读者对`pitaya`的工作
流程和工作机制有了一个初步的了解.
