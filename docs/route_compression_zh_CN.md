...
针对这种情况，`pitaya`提供了基于字典的路由信息压缩。

* 对于服务端，`pitaya`会扫描所有的Handler信息
* 对于客户端，用户需要配置一个路由映射表

通过这两种方式，`pitaya`会拿到所有的客户端和服务端的路由信息，然后将每一个路由信息都映射为一个小整数，从1
